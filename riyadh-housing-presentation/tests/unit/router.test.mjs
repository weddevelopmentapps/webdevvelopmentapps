...
import { RH, win, historyCalls } from "./load-app.mjs";
...
test("encodeReturn→decodeReturn: تقابل هوية على حالة عربية معقدة", () => {
  ...
  assert.deepStrictEqual(R.decodeReturn(R.encodeReturn(state)), state);
});
...
test("decodeReturn: سلسلة تالفة أو غير base64 أو JSON فاسد → null", () => {
  assert.strictEqual(R.decodeReturn("%%%غير-صالح%%%"), null);
  assert.strictEqual(R.decodeReturn("!!!"), null);
  assert.strictEqual(R.decodeReturn("YWJj"), null); // base64 صحيح لكن "abc" ليست JSON
  assert.strictEqual(R.decodeReturn(""), null);
});
...
test("التقابل يصمد عبر دورة العنوان كاملة (ترميز → مسار → تحليل → فك)", () => {
  ...
  assert.strictEqual(parsed.kind, "appendix");
  assert.strictEqual(parsed.id, "control");
  assert.strictEqual(parsed.params.page, "2");
  assert.deepStrictEqual(R.decodeReturn(parsed.params.return), state);
});
...
test("serialize→parse: تقابل على مسار مشهد بمعاملات متعددة", () => {
  ...
  assert.strictEqual(s, "/scene/05?step=2&sector=south");
  win.location.hash = "#" + s;
  assert.deepStrictEqual(R.parse(), route);
});
...
test("serialize: يسقط المعاملات الفارغة، و admin/home يختصر إلى /admin", () => {
  assert.strictEqual(
    ...
    "/scene/03");
  assert.strictEqual(R.serialize({ kind: "admin", id: "home", params: {} }), "/admin");
  assert.strictEqual(R.serialize({ kind: "admin", id: "publish", params: {} }), "/admin/publish");
});

test("parse: هاش فارغ أو جذر → المشهد 00", () => {
  win.location.hash = "";
  assert.deepStrictEqual(R.parse(), { kind: "scene", id: "00", params: {} });
  win.location.hash = "#/";
  assert.deepStrictEqual(R.parse(), { kind: "scene", id: "00", params: {} });
});
...
test("parse: ملحق بمعرف مركّب وadmin بتبويب", () => {
  ...
  assert.deepStrictEqual(R.parse(),
    { kind: "appendix", id: "lic/detail", params: { page: "1" } });
  win.location.hash = "#/admin";
  assert.deepStrictEqual(R.parse(), { kind: "admin", id: "home", params: {} });
});
...
test("المسارات القديمة تُستبدل (لا إدخال في التاريخ) وتصل لهدفها", () => {
  ...
  const route = R.parse();
  assert.strictEqual(route.kind, "scene");
  assert.strictEqual(route.id, "05");
  assert.strictEqual(historyCalls.length, before + 1, "استبدال واحد بالضبط");
  assert.strictEqual(historyCalls[historyCalls.length - 1], "#/scene/05");
  // الصيغة بلا شرطة مبدئية تُحوَّل أيضاً
  win.location.hash = "#summary";
  assert.strictEqual(R.parse().id, "02");
});
