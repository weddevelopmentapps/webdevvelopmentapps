...
RH.viz.geoutils = (function () {
  ...
  function normalizeAr(s) {
    return String(s || "")
      ...
      // التطبيع المسافي **قبل** حذف بادئة «حي»: استعلام المستخدم قد يبدأ
      // بفراغ («  حي الملقا») فتفشل المرساة ^ ويبقى «حي» في نص المطابقة.
      // (normDistrict أدناه يحتفظ بترتيب geomap الأصلي عمداً — مرآة حرفية.)
      .replace(/\s+/g, " ").trim()
      .replace(/^حي\s+/, "").trim();
  }
