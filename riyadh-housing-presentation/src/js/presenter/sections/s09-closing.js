   ...
   backdrop:"closing" → مستعار mediaBg إلى خلفية kpis_closing, major:true):
     ...
     ▸ صف التوصيات والأفق:
         ...
         • بطاقة «أفق المرحلة المقبلة»: donut حالات المبادرات المصغر
           ...
           + بطاقة «الخطوات التالية» الصادقة قيد الاعتماد (layout.pendingCard
...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     14) بطاقة أفق المرحلة المقبلة — donut حالات المبادرات المصغر + صفوف
         ...
         التالية» الصادقة قيد الاعتماد (بطاقة مدمجة — لا شاشة كاملة أبداً).
     ══════════════════════════════════════════════════════════════════════════ */
  function buildHorizonCell(cell, ctx, model) {
    ...
    /* الخطوات التالية — الحالة الصادقة قيد الاعتماد كبطاقة مدمجة */
    if (model.nextSteps) {
      RH.presenter.layout.pendingCard(cell, {
        label: "الخطوات التالية — " + statusBadgeText(model.nextSteps.status),
  ...
  function build(el, ctx) {
    ...
    /* الصف الثاني: التوصيات (7) + الأفق والخطوات التالية (5) */
