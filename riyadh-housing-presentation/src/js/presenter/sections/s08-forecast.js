...
(function () {
  ...
  function buildRail(cell, ctx, model, linker) {
    ...
    /* سطر تاريخ الحساب — من meta الإصدار حصراً. إصلاح مراجعة الجولة 2
       (استكمال): «البيانات حتى…» محذوفة هنا حتى في المقاس الكامل — كانت
       تطيل السطر فيلتف سطرين ويطبع سطره الثاني تحت حبة شارة الإصدار
       (التي تعلن «البيانات حتى…» نصاً بذاتها)؛ يبقى هنا تاريخ الحساب
       حصراً — المعلومة الوحيدة غير المكررة — سطراً واحداً لا يلتف. */
  ...
  function build(el, ctx) {
    ...
    /* نقطة الدخول المرخصة الوحيدة إلى مستكشف السيناريوهات (عقد التوسعة §4):
       زر واحد يفتح الملحق مع حفظ حالة القسم كاملة (عقد §7) — ولا تغيير آخر
       في القسم. موضعه فراغُ منتصف الترويسة (‎.dash-head‎ توزيعها
       space-between وارتفاعها الأدنى مضبوط سلفاً) فلا يقتطع بكسلاً واحداً
       من شبكة اللوحة المعتمدة: يُدرج **قبل** كتلة الوسم والسطر الثانوي
       فيبقى العنوان يميناً والوسم يساراً كما اعتمدهما المجلس تماماً.
       الزر يظهر فقط إذا كان الملحق مضمّناً في هذا البناء (البناء الجزئي
       أثناء التطوير لا يخلّف زراً معطلاً). */
    if (RH.explore && RH.explore.scenarios) {
      head.insertBefore(h("button", {
        class: "axs-entry",
        type: "button",
        "data-interactive": "",
        "aria-label": "فتح مستكشف السيناريوهات: منزلق الأشهر ودلتاتها "
          + "وترجمة كل عجز إلى نسبة تغطية مكافئة",
        onclick: () => ctx.openAppendix("scenarios"),
      },
        h("span", {}, "استكشاف السيناريوهات"),
        h("span", { class: "axs-entry-arrow", "aria-hidden": "true" }, "←"),
