...
(function () {
   ...
   * و‎countLabel‎ العدّ الظاهر الملزم («1 من 6»).
   *
   * ‏**العزل الاتجاهي يقع على كل رقم وحده، لا على الجملة.** الجملة عربية
   * فاتجاهها من اليمين إلى اليسار؛ ولو عُزلت كاملةً بعزل يساري (‎LRI‎) لقلب
   * محرّك ‎bidi‎ ترتيبها المرئي إلى «‎1 6 من‎» — عدّ مقلوب يقرؤه الحضور خطأً.
   * بعزل الأرقام منفردة يبقى ترتيب الجملة عربياً («‎1 من 6‎» قراءةً) ويبقى كل
   * رقم متماسكاً مهما حمل من فواصل آلاف.
   */
  function pillars(release) {
    ...
    return (st.pillars || []).map((p) => {
      ...
      return {
        ...
        /* «n من N» عبارة واحدة معزولة بـfmt.ofTotal — العزل الفردي لكل رقم
           كان يترك «من» خارج أي عزل فينقلب ترتيب العبارة في وعاء يساري. */
        countLabel: fmt.ofTotal(done, list.length),
      };
    });
  }
